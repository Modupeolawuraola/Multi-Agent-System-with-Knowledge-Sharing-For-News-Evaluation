...
import json
from src_v3.memory.schema import GraphState, NewsArticle
...
class FactCheckerAgent:
    ...
    def verify_direct_claim(self, claim_text: str) -> Dict[str, Any]:
        """
        Verify a user-submitted factual claim using knowledge graph information.
        """
        evidence = self.tools.search_for_evidence(claim_text)
        if evidence is None:
            evidence = []

        system_message = """You are a fact-checking assistant with access to a political knowledge graph.
        Your task is to verify the user's factual claim using available evidence. Consider consistency with known facts,
        trusted sources, and relationships in the knowledge graph.

        Return ONLY a JSON object with this format:
        {
            "is_verified": true/false,
            "reasoning": "explanation",
            "evidence_summary": "short summary of relevant evidence",
            "matched_nodes": ["list of node types that supported the verification"]
        }
        """
        ...
        try:
            ...
            logging.error(f"Error verifying direct claim: {e}")
            return {
                "is_verified": False,
...
def fact_checker_agent(state: GraphState, knowledge_graph) -> GraphState:
    ...
    if news_query:
        ...
        try:
            ...
            result = fact_checker.verify_direct_claim(news_query)

            # Create a temporary article from the query
            new_state.fact_check_result = {
                "claim": news_query,
                "result": result,
                "final_verdict": "True" if result.get("is_verified") else "False",
                "timestamp": datetime.now().isoformat()
            }
            new_state.current_status = 'fact_check_complete'

        except Exception as e:
            logging.error(f"Error in fact checking query: {e}")
    ...
    #
    # new_state.current_status = 'no_claim_provided'
    # return new_state
    #
