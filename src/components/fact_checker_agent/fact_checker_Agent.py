...
def fact_checker_agent(state: GraphState) -> GraphState:
    ...
    if news_query:
        ...
        try:
            ...
            knowledge_graph = state.knowledge_graph if hasattr(state, "knowledge_graph") else {}
            matched_articles = []

            if knowledge_graph and 'articles' in knowledge_graph:
                for article in knowledge_graph.get('articles', []):
                    if (news_query.lower() in article.get('title', '').lower() or
                            news_query.lower() in article.get('content', '').lower()):
